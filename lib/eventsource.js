...
function EventSource (url, eventSourceInitDict) {
  ...
  function connect () {
    ...
    // tunnel agent for https with proxy
    if (eventSourceInitDict && eventSourceInitDict.agent) {
  ...
  function parseEventStreamLine (buf, pos, fieldLength, lineLength) {
    if (lineLength === 0) {
      ...
      var comment = buf.slice(pos + 1, pos + lineLength);
      _emit('comment', comment);
    }
  }
}
...
['open', 'error', 'message', 'comment'].forEach(function (method) {
